...
simular_cartera <- function(retornos, capital_inicial, retiro_mensual, infla, costo_txn, 
  ...
  for (i in seq_along(months_vec)) {
    ...
    # Ajuste por inflación cada enero 
    if (mes_actual == 1 && i > 1) {
...
umbrales <- c(-0.05,0.03, 0.05, 0.10, 0.15)
...
# ======================================================================
# LITERAL 1 (gráficos comparativos alrededor del óptimo) 
# ======================================================================
...
# ======================================================================
# LITERAL 2 - Impacto de Inflación 
# ======================================================================
df_inf <- data.frame()
...
# ======================================================================
# GRÁFICA COMPARATIVA FINAL: FRECUENCIAS VS UMBRALES
# ======================================================================

df_comparativa_final <- data.frame()

# Estrategias seleccionadas para comparación
estrategias_comparar <- list(
  list(tipo = "frecuencia", valor = 1, nombre = "Mensual"),
  list(tipo = "frecuencia", valor = 3, nombre = "Trimestral"),
  list(tipo = "frecuencia", valor = 12, nombre = "Anual"),
  list(tipo = "frecuencia", valor = 24, nombre = "Bianual"),
  list(tipo = "umbral", valor = -0.05, nombre = "Umbral -5%"),
  list(tipo = "umbral", valor = 0.05, nombre = "Umbral 5%")
)

for (estrategia in estrategias_comparar) {
  if (estrategia$tipo == "frecuencia") {
    sim <- simular_cartera(retornos, capital_inicial, retiro_mensual, infla, costo_txn, pesos_base,
                           tipo_rebalanceo = "frecuencia", frecuencia = estrategia$valor)
  } else {
    sim <- simular_cartera(retornos, capital_inicial, retiro_mensual, infla, costo_txn, pesos_base,
                           tipo_rebalanceo = "umbral", umbral = estrategia$valor)
  }
  
  df_comparativa_final <- rbind(df_comparativa_final, data.frame(
    date = sim$months,
    capital = sim$capital[-1],
    estrategia = estrategia$nombre,
    tipo = ifelse(estrategia$tipo == "frecuencia", "Frecuencia", "Umbral")
  ))
}

g_comparativa_final <- ggplot(df_comparativa_final, aes(x = date, y = capital, color = estrategia, linetype = tipo)) +
  geom_line(size = 0.9) +
  geom_hline(yintercept = 0, linetype = "dashed", color = "red", alpha = 0.5) +
  labs(
    title = "Comparación de Estrategias de Rebalanceo",
    subtitle = "Frecuencias vs Umbrales Seleccionados",
    x = "Fecha",
    y = "Capital ($)",
    color = "Estrategia",
    linetype = "Tipo"
...
print(g_comparativa_final)

# Mostrar resultados numéricos de estas estrategias
...
for (estrategia in estrategias_comparar) {
  ...
  cat(sprintf("%-15s: %5.1f años | Capital Final: $%9s\n", 
              estrategia$nombre, meses/12, format(capital_final, big.mark = ",")))
}
